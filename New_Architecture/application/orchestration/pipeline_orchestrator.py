...
class PipelineOrchestrator:
    ...
    def __init__(
        ...
        self.pipeline_factory = pipeline_factory or PipelineFactory(service_factory)
    ...
    def _save_statistics(self, result: Dict[str, Any], config: PipelineConfiguration) -> None:
        ...
        try:
            ...
            if output_path.exists():
                ...
                stats_repo.save_statistics(
                    stats_df,
                    str(output_path),
                    format=config.output_format
                )
            
            logger.debug(f"Statistics saved to {output_path}")
    ...
    def _generate_report(self) -> None:
        ...
        partial = sum(1 for r in self.results if not r.get('success', False))
        ...
        if self.failed_configs:
            ...
            for config in self.failed_configs:
                logger.error(f"  - {config.cloud_pair.tag}")
        
        # Statistik-Zusammenfassung wenn vorhanden
        if any('statistics' in r for r in self.results):
            self._log_statistics_summary()
        
        logger.info("=" * 80)
    
    def _log_statistics_summary(self) -> None:
        """Loggt eine Zusammenfassung der Statistiken"""
        stats = [r['statistics'] for r in self.results if 'statistics' in r]
        ...
        metrics = ['mean', 'std', 'rms', 'mae', 'outlier_count']
        
        for metric in metrics:
            values = [s.get(metric) for s in stats if metric in s]
            if values:
                avg = np.mean(values)
                std = np.std(values)
                logger.info(f"  {metric}: avg={avg:.6f}, std={std:.6f}")
    ...
    def get_failed_configs(self) -> List[PipelineConfiguration]:
        """Gibt fehlgeschlagene Konfigurationen zurück"""
